...
void SimpleCompute::SetupSimplePipeline()
{
    ...
    vk_utils::allocateAndBindWithPadding(m_device, m_physicalDevice, { m_in, m_result }, 0);
    ...
    m_pBindings->BindEnd(&m_reductionDS, &m_reductionDSLayout);

    // Заполнение буферов
    m_pCopyHelper->UpdateBuffer(m_in, 0, m_input->data(), sizeof(float) * m_input->size());
}
...
void SimpleCompute::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkPipeline)
{
    ...
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_layout, 0, 1, &m_reductionDS, 0, NULL);
...
void SimpleCompute::CreateComputePipeline()
{
    ...
    pipelineLayoutCreateInfo.pSetLayouts    = &m_reductionDSLayout;
...
void SimpleCompute::Execute()
{
    ...
    *m_compute_time_out = elapsed_seconds.count();

    m_pCopyHelper->ReadBuffer(m_result, 0, m_output->data(), sizeof(float) * m_output->size());
}
